...
async function handleMenuFormSubmit(e) {
  ...
  if (isNaN(priceRegular) || isNaN(priceLarge) || priceRegular <= 0 || priceLarge <= 0) {
...
function bindMenuEvents() { 
    ...
    menuTableBody?.addEventListener('click', (e) => { 
        ...
        if (target.classList.contains('status-toggle')) toggleMenuAvailability(id, target.dataset.currentStatus === 'true'); 
...
async function hasOrderHistory(productId) { 
    const { count, error } = await supabase.from('supplement_order_items').select('id', { count: 'exact', head: true }).eq('supplement_product_id', productId); 
    if (error) { console.error('check history error:', error); return true; } 
    return (count ?? 0) > 0; 
}
...
async function openEditSupplementModal(id) { 
    ...
    if (currentImg) { currentImg.textContent = data.image_url ? `Current: ${data.image_url.split('/').pop()}` : 'No image uploaded.'; } 
    suppModal.classList.remove('hidden'); 
    window.lucide?.createIcons(); 
}
...
async function handleSupplementSubmit(e) { 
    ...
    const required = { name: gv('supp-name').trim(), category: gv('supp-category'), price: Number(gv('supp-price')), buying_price: Number(gv('supp-buying')), stock: Number(gv('supp-stock')) }; 
    ...
    if (imageFile) { 
        try { 
            ...
            const { data: uploadData, error: upErr } = await supabase.storage.from(SUPP_BUCKET).upload(filePath, imageFile, { cacheControl: '3600', upsert: false, contentType: imageFile.type || `image/${ext === 'jpg' ? 'jpeg' : ext}`, }); 
            if (upErr) throw upErr; 
            const { data: urlData } = supabase.storage.from(SUPP_BUCKET).getPublicUrl(uploadData.path); 
            imageUrl = urlData?.publicUrl || null; 
        } catch (err) { 
            console.error('Image upload failed:', err); 
            alert(err?.message?.includes('Bucket not found') ? 'Storage bucket not found. Create a PUBLIC bucket named "supplement-images" in Supabase.' : 'Image upload failed. Please try another image.'); 
            btn.disabled = false; btn.textContent = 'Save Product'; 
            return; 
        } 
    } 
    ...
    let error; 
    if (currentEditingSuppId) ({ error } = await supabase.from('supplement_products').update(payload).eq('id', currentEditingSuppId)); 
    else ({ error } = await supabase.from('supplement_products').insert([payload])); 
...
async function toggleSupplementAvailability(id, current) { 
    const { error } = await supabase.from('supplement_products').update({ available: !current }).eq('id', id); 
...
async function deleteSupplement(id) { 
    ...
    if (linked) { 
        if (!confirm('This product has order history.\nYou cannot delete it.\n\nMark it as "Unavailable" instead?')) return; 
        const { error } = await supabase.from('supplement_products').update({ available: false }).eq('id', id); 
        if (error) { console.error(error); alert('Failed to mark as unavailable.'); } 
        else { await loadSupplements(); } 
        return; 
    } 
    
    const { data: row, error: rowErr } = await supabase.from('supplement_products').select('image_url').eq('id', id).single(); 
    if (!rowErr && row?.image_url) { 
        const parsed = parseStoragePath(row.image_url); 
        if (parsed && parsed.bucket) { 
            try { await supabase.storage.from(parsed.bucket).remove([parsed.path]); } 
            catch (e) { console.warn('storage remove failed:', e); } 
        } 
    } 
    const { error } = await supabase.from('supplement_products').delete().eq('id', id); 
    if (error) { console.error(error); alert(error.code === '23503' || error.status === 409 ? 'Cannot delete: this product has related order records.' : 'Failed to delete product.'); } 
    else { await loadSupplements(); } 
}
...
async function loadDashboard() {
  ...
  const { data: todayOrders } = await supabase.from('orders').select('*').eq('status','Delivery Complete').gte('created_at', fromISO);
  const { data: todaySupp } = await supabase.from('supplement_orders').select('*').eq('status','Completed').gte('created_at', fromISO);
  const { data: pendingOrders } = await supabase.from('orders').select('*').neq('status', 'Delivery Complete');
  const { data: pendingSupp } = await supabase.from('supplement_orders').select('*').eq('status', 'Pending');
  const { data: weekOrders } = await supabase.from('orders').select('*').eq('status','Delivery Complete').gte('created_at', weekISO).lt('created_at', fromISO);
  const { data: weekSupp } = await supabase.from('supplement_orders').select('*').eq('status','Completed').gte('created_at', weekISO).lt('created_at', fromISO);

  const todayRevenue = [...(todayOrders||[]), ...(todaySupp||[])].reduce((s,o)=>s+Number(o.total_amount||0),0);
  const weekRevenue = [...(weekOrders||[]), ...(weekSupp||[])].reduce((s,o)=>s+Number(o.total_amount||0),0);
  const avgDaily = weekRevenue / 7;
  const revenueChange = avgDaily > 0 ? ((todayRevenue - avgDaily) / avgDaily * 100).toFixed(1) : 0;

  const totalPending = (pendingOrders?.length || 0) + (pendingSupp?.length || 0);
  const todayCompleted = (todayOrders?.length || 0) + (todaySupp?.length || 0);
  ...
  statsContainer.innerHTML = `
    <div class="stat-card green">
      ...
      <div class="stat-change ${revenueChange >= 0 ? 'positive' : 'negative'}">
        <i data-lucide="${revenueChange >= 0 ? 'trending-up' : 'trending-down'}"></i>
  ...
  if (recent.length) {
    recentPreview.innerHTML = '<div class="table-container"><div style="padding: 1rem;">' +
      recent.map(o => {
        const isSupp = !o.order_items;
        return `<div class="order-card" style="--status-color: var(--blue); margin-bottom: 1rem;">
          <div class="order-header">
            <div>
              <h3>${isSupp ? 'Supp' : 'Order'} #${String(o.id).slice(0,6).toUpperCase()}</h3>
              <p class="order-customer">${o.customer_name || 'Walk-in'}</p>
              <p class="order-time">${new Date(o.created_at).toLocaleTimeString()}</p>
            </div>
    ...
    recentPreview.innerHTML = '<p class="empty-message">No pending orders.</p>';
  }
...
async function fetchAndRenderTransactions() {
    ...
    if (transactionsList) {
      transactionsList.innerHTML = `
        <div class="unauthorized-view">
          ...
          <p>Staff users can only view current date transactions.</p>
        </div>
      `;
      window.lucide?.createIcons();
    }
  ...
  let cafeQuery = supabase.from('orders').select('*, order_items(*)').eq('status','Delivery Complete').gte('created_at', fromISO);
  let suppQuery = supabase.from('supplement_orders').select('*, supplement_order_items(*)').eq('status','Completed').gte('created_at', fromISO);
  ...
  const { data: cafeTx } = await cafeQuery.order('created_at',{ascending:false});
  const { data: suppTx } = await suppQuery.order('created_at',{ascending:false});

  let cafe = (cafeTx || []).map(t => ({...t, type: 'cafe'}));
  let supp = (suppTx || []).map(t => ({...t, type: 'supplement'}));
  ...
  allTransactions = [...cafe, ...supp].sort((a,b) => new Date(b.created_at) - new Date(a.created_at));

  const totalRevenue = allTransactions.reduce((s,o)=>s+Number(o.total_amount||0),0);
  ...
  if(transactionsSummary) {
    transactionsSummary.innerHTML = `
      <div class="summary-item"><h4>Total Revenue</h4><p>${fmtBDT(totalRevenue)}</p></div>
      <div class="summary-item"><h4>Total Orders</h4><p>${completedCount}</p></div>
      <div class="summary-item"><h4>Average Order</h4><p>${fmtBDT(avgTransaction)}</p></div>
    `;
  }
  ...
  if (!cafe.length) cafeCol.innerHTML += `<p class="empty-message">No cafe transactions.</p>`;
  else cafe.forEach(tx=>{
    ...
    el.innerHTML = `
      <div class="transaction-details">
        <p><strong>${tx.customer_name}</strong> – Order #${String(tx.id).slice(0,6).toUpperCase()}</p>
  ...
  if (!supp.length) suppCol.innerHTML += `<p class="empty-message">No supplements transactions.</p>`;
  else supp.forEach(tx=>{
    const el=document.createElement('div');
    el.className='transaction-item transaction-card';
    el.dataset.txId = tx.id;
    ...
    el.innerHTML = `
      <div class="transaction-details">
        <p><strong>${tx.customer_name || 'Walk-in'}</strong> – Supp #${String(tx.id).slice(0,6).toUpperCase()}</p>
        <p class="order-time">${new Date(tx.created_at).toLocaleString()}</p>
      </div>
      <p class="transaction-amount">${fmtBDT(tx.total_amount)}</p>
    `;
  ...
  if(loadingTransactions) loadingTransactions.classList.add('hidden');
  if (!cafe.length && !supp.length && noTransactionsMsg) noTransactionsMsg.classList.remove('hidden');
}

function showTransactionDetail(txId, txType) {
  const tx = allTransactions.find(t => String(t.id) === String(txId) && t.type === txType);
  if (!tx) return;

  const items = txType === 'cafe' ? (tx.order_items || []) : (tx.supplement_order_items || []);
      ...
      <div class="detail-section">
        ...
        <div class="detail-grid">
          <div class="detail-item">
            ...
            <div class="detail-value">#${String(tx.id).slice(0,8).toUpperCase()}</div>
          </div>
          <div class="detail-item">
            <div class="detail-label">Type</div>
            <div class="detail-value">${txType === 'cafe' ? 'Cafe Menu' : 'Supplements'}</div>
          </div>
          <div class="detail-item">
            <div class="detail-label">Customer</div>
            <div class="detail-value">${tx.customer_name || 'Walk-in'}</div>
          </div>
          <div class="detail-item">
...
function exportTransactionsToCSV() {
  ...
  const headers = ['Date', 'Time', 'Order ID', 'Type', 'Customer', 'Amount', 'Items'];
  const rows = allTransactions.map(tx => {
    const items = tx.type === 'cafe'
      ? (tx.order_items || []).map(i => `${i.quantity}x ${i.item_name}`).join('; ')
      : (tx.supplement_order_items || []).map(i => `${i.quantity}x ${i.item_name}`).join('; ');
...
function bindTransactionEvents() {
  $('#apply-filters-btn')?.addEventListener('click', fetchAndRenderTransactions);
  $('#export-transactions-btn')?.addEventListener('click', exportTransactionsToCSV);
  $('#close-transaction-detail-btn')?.addEventListener('click', () => transactionDetailModal?.classList.add('hidden'));
  $('#close-detail-btn')?.addEventListener('click', () => transactionDetailModal?.classList.add('hidden'));
  ...
  data.forEach(req => {
    ...
    card.innerHTML = `
      <div class="request-header">
        <div>
          <h4 style="font-size: 1.1rem; margin-bottom: 0.5rem;">${req.supplement_name}</h4>
          <p class="order-time">${new Date(req.created_at).toLocaleString()}</p>
        </div>
        <span class="request-status ${req.status}">${req.status.charAt(0).toUpperCase() + req.status.slice(1)}</span>
      </div>
